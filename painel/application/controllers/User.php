...
*/
     ...
     /*   if ($this->getUserStatus() == TRUE)
        {
            $this->session->set_flashdata('error', 'Altere sua senha para sua segurança primeiro.');
    ...
    {
        $this->global['pageTitle'] = 'ADMIN: configurações da conta';
    ...
    function updateUser()
    {
        ...
        else
        {
            ...
            if($result == true)
            {
                ...
                $this->session->set_flashdata('success', 'As configurações da sua conta foram atualizadas com sucesso');
            }
            else
            {
                $this->session->set_flashdata('error', 'Falha na atualização das configurações da conta');
            }
    ...
    {
        $this->global['pageTitle'] = 'ADMIN: Alterar senha';
    ...
    {
        $this->global['pageTitle'] = 'ADMIN: 404 - Página não encontrada';
    ...
    function endTask($taskId)
    {
            ...
            if ($result > 0) {
                 ...
                 $this->logrecord($process,$processFunction);
                 $this->session->set_flashdata('success', 'Missão concluída com sucesso');
                ...
                $this->session->set_flashdata('error', 'Falha na conclusão da tarefa');
                if ($this->role != ROLE_EMPLOYEE){
                    redirect('tasks');
                 }
                 else{
    ...
    function etasks()
    {
            ...
            $process = 'Usuário Todas as tarefas';
